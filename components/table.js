...
const useSortableData = (items, config = null) => {
  ...
  const sortedRestaurants = React.useMemo(() => {
    ...
    if (sortMethod !== null) {
      ...
      if (sortMethod.stateFilter && sortMethod.stateFilter !== "all") {
        let pickArray = sortableRestaurants.filter((item) =>
          item["state"].includes(sortMethod.stateFilter)
        );
      ...
      if (sortMethod.genreFilter && sortMethod.genreFilter !== "all") {
        let pickArray = sortableRestaurants.filter((item) =>
          item["genre"].includes(sortMethod.genreFilter)
        );
        if (pickArray.length > 0) {
          sortableRestaurants = pickArray;
        } else {
        ...
        let pickArray = sortableRestaurants.filter((restaurant) => {
          for (const key in restaurant) {
            if (
              restaurant[key]
                .toLowerCase()
                .includes(sortMethod.searchValue.toLowerCase())
            ) {
              return true;
            }
          }
  ...
  const changeSortMethod = (key) => {
    ...
    if (
      sortMethod &&
      sortMethod.sort.key === key &&
      sortMethod.sort.direction === "ascending"
    ) {
...
function Table({ restaurants }) {
  ...
  const getClassNamesFor = (name) => {
    ...
    return sortMethod.key === name
      ? `${sortMethod.direction} ${styles.sortable} `
      : styles.sortable;
  };
        ...
        <tr>
          <th>
            <u
              ...
              onClick={(e) => changeSortMethod("name")}
            >
              Name
            </u>
          </th>
          <th>
            <u
              ...
              onClick={(e) => changeSortMethod("city")}
            >
              City
            </u>
          </th>
          <th>
            <div className={"d-flex"}>
              <u
                ...
                onClick={(e) => changeSortMethod("state")}
              >
                State
              </u>
              <SelectState stateSelected={stateSelected} />
            </div>
          </th>
          <th>Phone Number</th>
          <th>
